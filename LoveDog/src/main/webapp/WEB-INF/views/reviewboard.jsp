...
// 		alert(objidx.options[objidx.selectedIndex].value);
	...
	function writeForm(){
		location.href="./write.do?selEtc=R";
	}
...
//	function delflag() {
//		location.href = "./delflag.do?selEtc=R";
//	}
	
	function goPage(pageNo){
		document.listForm.pageNo.value = pageNo;
		document.listForm.selEtc.value = "R";
...
<form action="./delflag.do" method="post" id="frm" name="frm" onsubmit="return chkbox()">
	<div class="panel-group" id="accordion">
				<input type="hidden" name="selEtc" id="selEtc" value="R" onload="selView()">
   ...
   <tbody>
      <tr>
		...
		<td><input type="checkbox" name="chkVal" value="<%=dto.getSeq()%>"></td>
		 ...
		 <td><%=dto.getSeq()%></td>
         <td><%=dto.getU_id()%></td>
		<td><c:set var="dep" value="<%=dto.getDepth()%>" /> <c:if
			test="${dep > 0}">
		...
		<a href="detail.do?seq=<%=dto.getSeq()%>"><%=dto.getTitle()%></a></td>
	...
	<c:if test="${user.u_auth eq 'A'}">
		...
		<!-- 페이징 -->
		<form name="listForm" action="./reviewboard.do" method="get" style="position: relative; left: 780px; top: 180px;">
			<input type="hidden" name="pageNo" value="" />
			<input type="hidden" name="selEtc" value="R" /> <br/><br/>
...
<form name="form1" action="./reviewboard.do" method="get" style="position: relative; left: 750px; top :180px;">
