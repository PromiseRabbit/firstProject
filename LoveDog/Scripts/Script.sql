...
VALUES(POSTBOARD_SEQ.NEXTVAL, 'jjjj','제목','내용',(SELECT NVL(MAX(REFER),0)+1 FROM POSTBOARD),
...
UPDATE POSTBOARD
SET STEP=STEP+1
WHERE REFER = (SELECT REFER FROM POSTBOARD WHERE SEQ='1')
AND STEP > (SELECT STEP FROM POSTBOARD WHERE SEQ='1');	

--답글 (INSERT)
INSERT INTO POSTBOARD
(SEQ, U_ID, TITLE, CONTENT, REFER, STEP, DEPTH,  DELFLAG, REGDATE, READCOUNT,B_TYPE)
VALUES(POSTBOARD_SEQ.NEXTVAL, 'username', '제목이요', '답글작성', (SELECT REFER FROM POSTBOARD 
WHERE SEQ='1'), (SELECT STEP+1 FROM POSTBOARD WHERE SEQ = '1'), 
(SELECT DEPTH+1 FROM POSTBOARD WHERE SEQ='1'),'N',SYSDATE,0,'R');
